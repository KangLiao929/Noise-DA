...
def tensor2img(tensor, norm="norm0", out_type=np.uint8, min_max=(-1, 1)):
    '''
    ...
    if out_type == np.uint8:
            img_np = (img_np*255.).round() if(norm=="norm0") else ((img_np+1.)*127.5).round()
    
    return img_np.astype(out_type).squeeze()

def postprocess(images, norm="norm0"):
    return [tensor2img(image, norm) for image in images]
