...
class BaseModel():
    def __init__(self, opt, phase_loader, val_loader, phase_dataset, metrics, logger, writer): 
        ...
        self.epoch = 0
        self.iter = 0 
        self.diff_flag = self.opt['diff_flag']
        self.ctrl_epoch = self.opt['ctrl_epoch']
        self.diff_weight = self.opt['diff_weight']
        self.real_denoise = self.opt['datasets'][self.phase]['which_dataset']['args']['real_denoise']
        self.save_path = self.opt['path']['results']

        self.phase_loader = phase_loader
        self.val_loader = val_loader
        self.phase_dataset = phase_dataset
        self.metrics = metrics
        self.norm = self.opt['datasets'][self.phase]['which_dataset']['args']['norm']
    ...
    def train(self):
        while self.epoch <= self.opt['train']['n_epoch'] and self.iter <= self.opt['train']['n_iter']:
            ...
            if(self.opt['global_rank'] == 0):
                '''randomly shuffle the real-world and reference datasets to avoid the shortcut'''
                self.phase_dataset.shuffle_paths()
            train_log = self.train_step(self.epoch)
    ...
    @abstractmethod
    def train_step_scheduler(self, epoch):
        raise NotImplementedError('You must specify how to train your networks.')
    ...
    def is_empty_directory(self, path):
    ...
    def load_network(self, network, network_label, strict=True):
        if self.opt['path']['checkpoint'] is None or self.is_empty_directory(self.opt['path']['checkpoint']):
            return
        self.logger.info('Beign loading pretrained model [{:s}] ...'.format(network_label))

        model_path = self.opt['path']['checkpoint']
    ...
    def resume_training(self, network, network_label, strict=True):
        ...
        model_path = "{}_Network.pth".format(self. opt['path']['resume_state'])
        self.logger.info('Beign loading pretrained model [{:s}] ...'.format(network_label))
