...
def parse(args, now_model):
    ...
    if(now_model > 0):
        experiments_root = os.path.join(opt['path']['base_dir'], '{}_{}'.format('test', now_model))
    else:
    ...
    for key, path in opt['path'].items():
        if opt['phase']=='train':
            if 'resume' not in key and 'base' not in key and 'root' not in key and 'now_model' not in key:
                opt['path'][key] = os.path.join(experiments_root, path)
                mkdirs(opt['path'][key])
        else:
            if 'checkpoint' not in key and 'base' not in key and 'root' not in key and 'now_model' not in key:
                opt['path'][key] = os.path.join(experiments_root, path)
                mkdirs(opt['path'][key])
    # ''' debug mode '''
