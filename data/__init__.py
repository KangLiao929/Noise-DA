...
def define_dataloader(logger, opt):
    ...
    if opt['global_rank']==0 and val_dataset is not None:
        ...
        val_dataloader = None
    return dataloader, val_dataloader, phase_dataset
...
def define_dataset(logger, opt):
    ...
    dataset_opt_val = opt['datasets'][opt['phase']]['which_dataset_val']

    phase_dataset = init_obj(dataset_opt, logger, default_file_name='data.dataset', init_type='Dataset')
    val_dataset = init_obj(dataset_opt_val, logger, default_file_name='data.dataset', init_type='Dataset')
    valid_len = 0
